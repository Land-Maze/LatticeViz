...
\textbf{Chapter 2 (Chosen Methods)} details the algorithmic optimizations, specifically focusing on the Number Theoretic Transform (NTT) and the requirements for SIMD-accelerated arithmetic.
\textbf{Chapter 3 (Implementation)} describes the software architecture of the developed C library, the creation of the C++ benchmark \& tests.
\textbf{Chapter 4 (Performance Evaluation)} presents the results of benchmarks conducted on modern hardware, comparing the optimized implementation against reference standards.
Finally, \textbf{Chapter 5 (Conclusion)} summarizes the findings and discusses potential vectors for future research in post-quantum implementations.